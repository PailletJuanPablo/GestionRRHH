    ...
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        <div class="info-box-2 bg-indigo">
            ...
            <div class="content">
                <div class="text"></div>
                <div class="number ">
                    <h3 class="title">Empleado: {{$empleado->apellido_nombre}} </h3>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        <div class="info-box-2 bg-lime">
            ...
            <div class="content">
                ...
                <div class="number ">
                    <h3 class="title">
                        Días restantes: {{$empleado->diasLactancia()}}
                    </h3>
                </div>
            </div>
        </div>
    </div>
    @endif @if ($empleado->maternidad === 1)
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        <div class="info-box-2 bg-pink">
            ...
            <div class="content">
                ...
                <div class="number ">
                    <h3 class="title">
                        Días restantes: {{$empleado->diasMaternidad()}}
                    </h3>
                </div>
            </div>
        </div>
...
<div class="card">
    <div class="header bg-cyan">
        ...
        <h5> Horas Extras Tomadas: {{$horasExtra->sum("cantidad")}}</h5>
        <h5> Salidas Particulares realizadas: {{$empleado->salidasParticulares()->sum("cantidad_horas")}}</h5>
        <h3> HORAS DISPONIBLES: {{ round($horasExtra->sum("cantidad") - $empleado->salidasParticulares()->sum("cantidad_horas")
            - ($empleado->francosCompensatorios()->count()/ 6),0) }}

        </h3>
        <h3> DIAS FRANCOS COMPENSATORIOS DISPONIBLES: {{($horasExtra->sum("cantidad") - ($empleado->salidasParticulares()->sum("cantidad_horas"))/6
            ) - $empleado->francosCompensatorios()->count() }}</h3>
    </div>
...
@endif @if ($empleado->salidasParticulares->count() > 0)
<div class="card">
    <div class="header bg-red">
        <h2 class="title">Salidas particulares del empleado</h2>
        <h4> Total de Salidas Particulares registradas: {{$empleado->salidasParticulares->count()}}</h4>
    </div>
    <div class="body table-responsive">
        <table class="table table-responsive table-striped">
            <thead>
            ...
            <tbody>
                @foreach($empleado->salidasParticulares as $salida)
                <tr>
                    <td>
                        {{$salida->fecha}}
                    </td>
                    <td>
                        {{$salida->cantidad_horas}}
                    </td>
                </tr>
                @endforeach
            </tbody>
...
@endif @foreach ($ausencias as $ausencia_titulo => $ausencias)
<div class="card">
    <div class="header bg-cyan">
    ...
    <div class="body table-responsive">
        <table class="table table-responsive table-striped">
            ...
            <tbody>
                ...
                <tr>
                    <td>
                        @if ($ausencia->ausencia_multiple == 1) Desde {{$ausencia->inicio_ausencia}} hasta {{$ausencia->finalizacion_ausencia}} @else
                        {{$ausencia->fecha_ausencia}} @endif
                    </td>
                    <td>{{$ausencia->observaciones}} </td>
                    <td>@if ($ausencia->justificado == 1) SI @else NO @endif
                    </td>
                </tr>
                @endforeach
            </tbody>
